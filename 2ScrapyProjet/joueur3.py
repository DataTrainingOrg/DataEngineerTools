...
class TransfermarktPlayerSpider(scrapy.Spider):
    ...
    def parse_player_info(self, response):
        ...
        # Extraire les informations de la carrière internationale
        national_career = []
    ...
    def save_to_json(self):
        ...
        # Identifier les player_id manquants dans player_info
        all_ids = set(range(1, 101))  # IDs allant de 1 à 100
        missing_player_info_ids = all_ids - player_info_ids
        if missing_player_info_ids:
            print(f"Player IDs manquants dans player_info.json: {sorted(missing_player_info_ids)}")
        ...
        # Identifier les player_id manquants dans career_performance
        missing_career_performance_ids = all_ids - career_performance_ids
        if missing_career_performance_ids:
            print(f"Player IDs manquants dans career_performance.json: {sorted(missing_career_performance_ids)}")
