...
def scrape_products_info(product_name):
    ...
    if response.status_code == 200:
        ...
        for product in product_containers:
            title_element = product.find("a", class_="a-link-normal s-no-outline")
            title = title_element.find("img")["alt"] if title_element and title_element.find("img") else None
            ...
            product_url = f"https://www.amazon.fr/dp/{asin}" if asin else None
        ...
        return {"count": len(results), "products": results, "message": f"Récupération de la page réussie pour {product_name} avec User-Agent: {headers['User-Agent']}"}

    return {"count": 0, "products": [], "message": f"Échec de la récupération de la page pour {product_name}. Code d'état : {response.status_code}."}