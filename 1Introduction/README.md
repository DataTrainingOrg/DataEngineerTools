...
![Inspecteur Chrome](images/inspecteur.png)
...
- Redis (Base de données noSQL, stockage sous la forme de données clé:valeurs avec très grandes performances car tourne sur la RAM)
- ElasticSearch, même s'il ne s'agit pas réellement d'une base de données, c'est un moteur de recherche qui permet de faire de la recherche très efficacement dans des données textuelles, numériques et géolocalisées.
